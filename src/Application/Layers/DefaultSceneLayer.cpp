...
DefaultSceneLayer::DefaultSceneLayer() :
	ApplicationLayer()
{
	Name = "100788603 Kevin Huang Assignment 2";
...
void DefaultSceneLayer::_CreateScene()
{
	...
	if (loadScene && std::filesystem::exists("scene.json")) {
		...
		MeshResource::Sptr houseMesh = ResourceManager::CreateAsset<MeshResource>("house.obj");
		MeshResource::Sptr planeMesh = ResourceManager::CreateAsset<MeshResource>("plane.obj");
		MeshResource::Sptr crateMesh = ResourceManager::CreateAsset<MeshResource>("Crate.obj");
		...
		Texture3D::Sptr lut = ResourceManager::CreateAsset<Texture3D>("luts/indigo.CUBE");
		...
		// This will be the reflective material, we'll make the whole thing 90% reflective
		Material::Sptr planematerial1 = ResourceManager::CreateAsset<Material>(reflectiveShader);
		{
			planematerial1->Name = "plane";
			planematerial1->Set("u_Material.Diffuse", monkeyTex);
			planematerial1->Set("u_Material.Shininess", 0.5f);
		}

		// This will be the reflective material, we'll make the whole thing 90% reflective
		Material::Sptr housematerial1 = ResourceManager::CreateAsset<Material>(reflectiveShader);
		{
			housematerial1->Name = "house";
			housematerial1->Set("u_Material.Diffuse", monkeyTex);
			housematerial1->Set("u_Material.Shininess", 0.5f);
		}
		
		// This will be the reflective material, we'll make the whole thing 90% reflective
		Material::Sptr cratematerial1 = ResourceManager::CreateAsset<Material>(reflectiveShader);
		{
			cratematerial1->Name = "Crate";
			cratematerial1->Set("u_Material.Diffuse", monkeyTex);
			cratematerial1->Set("u_Material.Shininess", 0.5f);
		}
		...
		MeshResource::Sptr planeMesh1 = ResourceManager::CreateAsset<MeshResource>();
		planeMesh1->AddParam(MeshBuilderParam::CreatePlane(ZERO, UNIT_Z, UNIT_X, glm::vec2(1.0f)));
		planeMesh1->GenerateMesh();
		...
		{
			camera->SetPostion({ 6.310, -6.620, 22.870 });
		...
		GameObject::Sptr house = scene->CreateGameObject("house");
		{
			// Set position in the scene
			house->SetPostion(glm::vec3(1.520f, 9.500f, 5.170f));
			house->SetRotation(glm::vec3(90.0f, -3.0f, 0.0f));
			house->SetScale(glm::vec3(1.0f, 1.0f, 1.0f));

			// Add some behaviour that relies on the physics body
			//house->Add<JumpBehaviour>();

			// Create and attach a renderer for the monkey
			RenderComponent::Sptr renderer = house->Add<RenderComponent>();
			renderer->SetMesh(houseMesh);
			renderer->SetMaterial(housematerial1);

			// Example of a trigger that interacts with static and kinematic bodies as well as dynamic bodies
			//TriggerVolume::Sptr trigger = house->Add<TriggerVolume>();
			//trigger->SetFlags(TriggerTypeFlags::Statics | TriggerTypeFlags::Kinematics);
			//trigger->AddCollider(BoxCollider::Create(glm::vec3(1.0f)));

			//house->Add<TriggerVolumeEnterBehaviour>();
		}

		GameObject::Sptr plane1 = scene->CreateGameObject("plane1");
		{
			// Set position in the scene
			plane1->SetPostion(glm::vec3(4.150f, 14.860f, 1.080f));
			plane1->SetRotation(glm::vec3(90.0f, -3.0f, 0.0f));
			plane1->SetScale(glm::vec3(1.0f, 1.0f, 1.0f));

			// Add some behaviour that relies on the physics body
			//plane->Add<JumpBehaviour>();

			// Create and attach a renderer for the monkey
			RenderComponent::Sptr renderer = plane1->Add<RenderComponent>();
			renderer->SetMesh(planeMesh);
			renderer->SetMaterial(planematerial1);
		...
		GameObject::Sptr crate = scene->CreateGameObject("crate");
		{
			// Set position in the scene
			crate->SetPostion(glm::vec3(10.040f, 11.140f, 5.610f));
			crate->SetRotation(glm::vec3(90.0f, -3.0f, 0.0f));
			crate->SetScale(glm::vec3(1.0f, 1.0f, 1.0f));
			...
			renderer->SetMaterial(cratematerial1);
